...
TeleCodex can run as a macOS user LaunchAgent. This is the macOS equivalent of the Linux user systemd service: it runs under your user account, can reuse your Codex CLI auth state, and can access the same workspace files and developer tools available on the host. Codex CLI/app-server 0.144.1 is the minimum compatible baseline; 0.151.0 is the recommended stable version.
